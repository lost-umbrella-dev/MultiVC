...
crates/
  ...
  composer/  — Core business logic: Lock trait, installation pipeline, worker/channel protocol, instance management, AppPaths (paths.rs)
  cli/       — clap CLI; calls Composer directly (no channels); supports --portable flag
...
### Lock Trait (composer)
`CoresLock` implements the `Lock` trait, which handles TOML persistence (`load`/`save`/`remove`/`validate_dir`). Each lock file lives at `{data_dir}/cores/lock.toml` and `{data_dir}/instances/lock.toml`, where `data_dir` comes from `AppPaths`. `LockMap` is a `DashMap<Hash, LockItem>` used for concurrent access.
...
### i18n (gui)
`lang.rs` provides `t(key, lang) -> &'static str` for EN/RU translations. Language is persisted in `settings.toml` at the config directory (`AppPaths.config_dir`).
...
- **Tests:** integration tests under `crates/{crate}/tests/`; use `AppPaths` with `tempfile::TempDir` for isolation (no CWD mutation); init tracing via `init_test_tracing()` from `tests/common/mod.rs`
