...
MultiVC управляет ядрами [VoxelCore](https://github.com/MihailRis/voxelcore) как зависимостями: каждая версия ядра устанавливается один раз, а инстансы (игровые миры) ссылаются на нужную версию по хэшу. Данные хранятся в пользовательских директориях (конфиг и данные отдельно), без записи в реестр. Поддерживается portable-режим — если `settings.toml` лежит рядом с исполняемым файлом, всё хранится там же.
...
По умолчанию MultiVC хранит данные в пользовательских директориях:
...
В конфиге хранится `settings.toml` (язык, `[paths]` и другие настройки). В данных — `cores/` и `instances/`.

**Portable-режим** — если `settings.toml` находится рядом с исполняемым файлом, всё хранится там же. Принудительно активировать: `--portable`.

Структура данных (одинакова для обоих режимов):
...
<config dir>/
└── settings.toml         # настройки GUI (язык, [paths])
```

Пользовательские пути можно задать через секцию `[paths]` в `settings.toml`.
...
Готовые бинарники на странице [Releases](https://github.com/lost-umbrella-dev/MultiVC/releases). Архивы включают `settings.toml` рядом с исполняемым файлом — portable-режим активен сразу после распаковки.
