...
pub fn hash_directory(
    path: &Path,
    hash: &Hash,
) -> std::result::Result<bool, HashError> {
...
pub fn hash_directory_into(
    ...
    for entry in entries {
        let path = entry.path();
        let relative = path.strip_prefix(root).map_err(|source| HashError::Read {
            path: path.clone(),
            source: std::io::Error::other(source),
        })?;

        let file_type = entry.file_type().map_err(|source| HashError::Read {
            path: path.clone(),
            source,
        })?;
        ...
        if file_type.is_dir() {
            ...
            let mut file = std::fs::File::open(&path).map_err(|source| HashError::Read {
                path: path.clone(),
                source,
            })?;
